...
import { FacultyProfileCard } from "@/components/layout/FacultyProfileCard";
import { FACULTY_NAV } from "@/constants/faculty-nav";
...
export function FacultyLayout({ children }: FacultyLayoutProps) {
  ...
  if (user.baseProfile !== "faculty") return <Navigate to="/login" />;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-page">
      ...
      <div className="flex flex-1">
        {profile ? (
          <AppSidebar
            sections={FACULTY_NAV}
            header={<FacultyProfileCard profile={profile} />}
            open={sidebarOpen}
          />
        ) : (
